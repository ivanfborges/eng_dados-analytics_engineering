...
Atualização 3.2: divisão criada e congelada nos [metadados](eda/partitions.json). A EDA somente do treino e a validação espacial estão em [EDA.pt-BR.md](EDA.pt-BR.md). A presença de alvo válido define elegibilidade; atributos ausentes não removem anúncios. Atualização 3.3a: dois baselines de mediana avaliados em folds do treino; ver [BASELINES.pt-BR.md](BASELINES.pt-BR.md) para o protocolo de comparação congelado. Etapa 3.3b: quatro candidatos comparados, histgb_geo selecionado e validação externa avaliada uma vez. Ver [SELECTION.pt-BR.md](SELECTION.pt-BR.md). Artefato treinado congelado; teste final reservado.
